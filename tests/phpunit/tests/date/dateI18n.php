...
class Tests_Date_I18n extends WP_UnitTestCase {
	...
	public function test_should_format_date() {
		$this->assertEqualsWithDelta( strtotime( gmdate( 'Y-m-d H:i:s' ) ), strtotime( date_i18n( 'Y-m-d H:i:s' ) ), 'The dates should be equal', 2 );
	}
	...
	public function test_date_should_be_in_gmt() {
		$this->assertEqualsWithDelta( strtotime( gmdate( DATE_RFC3339 ) ), strtotime( date_i18n( DATE_RFC3339, false, true ) ), 'The dates should be equal', 2 );
	}

	public function test_custom_timezone_setting() {
		update_option( 'timezone_string', 'America/Regina' );

		$this->assertEqualsWithDelta( strtotime( gmdate( 'Y-m-d H:i:s', time() + get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) ), strtotime( date_i18n( 'Y-m-d H:i:s' ) ), 'The dates should be equal', 2 );
	}
	...
	public function test_date_i18n_handles_shorthand_formats( $short, $full ) {
		...
		$this->assertEqualsWithDelta( strtotime( date_i18n( $full ) ), strtotime( date_i18n( $short ) ), 'The dates should be equal', 2 );
	...
	public function test_should_return_wp_timestamp() {
		...
		$this->assertEqualsWithDelta( $wp_timestamp, date_i18n( 'U' ), 'The dates should be equal', 2 );
		$this->assertEqualsWithDelta( $timestamp, date_i18n( 'U', false, true ), 'The dates should be equal', 2 );
